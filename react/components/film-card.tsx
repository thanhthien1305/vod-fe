...
export default function CardFilm({ film }: FilmCardProps) {
  return (
    <div className="relative overflow-hidden cursor-pointer group w-fit">
      ...
      />
      <div className="absolute text-bold-title2 font-bold bottom-0 left-0 w-full  bg-opacity-60 text-white text-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-2 text-sm">
        {film?.title || 'Tên phim'}
      </div>
    </div>
  );
